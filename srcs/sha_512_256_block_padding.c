...
declare_block_padding_functions(sha_512_256, SHA_512_256, schedule.X)

static void 	size_padding(t_sha_512_256_state* state) {
	if (!do_size_padding(state)) { return; }
	const uint64_t s_64 = state->message_len;
	const uint64_t	size_from = SHA_512_256_WORD_BLOCK_BIT_SIZE - sizeof(uint64_t) * OCTET_BIT_SIZE;
