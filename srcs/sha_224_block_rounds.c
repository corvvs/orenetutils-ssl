...
#define SHR(n, x) ((x) >> (n))
#define ROTR(n, x) (((x) >> (n)) | ((x) << (SHA_224_WORD_BIT_SIZE - (n))))
#define ROTL(n, x) (((x) << (n)) | ((x) >> (SHA_224_WORD_BIT_SIZE - (n))))
...
#define OPE(a, b, c, d, e, f, g, h, W, i)     \
	{                                         \
		sha_224_word_t T1 = h + BSIG1(e) + CH(e, f, g) + SHA_224_K[i] + W[i]; \
		sha_224_word_t T2 = BSIG0(a) + MAJ(a, b, c); \
		h = g;\
		g = f;\
		f = e;\
		e = d + T1;\
