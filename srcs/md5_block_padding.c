...
// src から dst へビット単位でデータをコピーする.
// コピー元: [src + bit_offset, src + bit_offset + bit_len)
// コピー先: [dst + bit_offset, dst + bit_offset + bit_len)
void	bitwise_memcpy(uint8_t* dst, const uint8_t* src, uint64_t bit_offset, uint64_t bit_len) {
	...
	if (bit_to < OCTET_BIT_SIZE) {
		subbyte_memcpy(dst, src, bit_from, bit_to);
		return;
	}
	...
	if (bit_from % OCTET_BIT_SIZE > 0) {
		...
		subbyte_memcpy(&dst[byte_index], &src[byte_index], OCTET_BIT_SIZE, bit_from);
	}

	// バイトコピー
	if (byte_to > byte_from) {
		ft_memcpy(&dst[byte_from], &src[byte_from], byte_to - byte_from);
	}
	
	// 後のサブバイトコピー
	if (bit_to % OCTET_BIT_SIZE > 0) {
		const uint64_t	byte_index = bit_to / OCTET_BIT_SIZE;
		subbyte_memcpy(&dst[byte_index], &src[byte_index], bit_to, 0);
	}
}
...
// セット値: bit
void	bitwise_memset(uint8_t* mem, const uint8_t bit, uint64_t bit_offset, uint64_t bit_len) {
...
bool	do_message_copy(const t_md5_state* state) {
	return state->block_from < state->message_len;
}

void	copy_message_data(t_md5_state* state) {
...
bool	do_size_padding(const t_md5_state* state) {
...
void 	size_padding(t_md5_state* state) {
...
void	print_byte_array(const uint8_t* bytes, size_t len) {
